...
OUTPUTS_ROOT = DEFAULT_ROOT
CACHE_PATH = DEFAULT_ROOT / ".dashboard-cache.json"
CACHE_VERSION = 4
    ...
    def walk(directory: Path) -> dict:
        ...
        for child in sorted(directory.iterdir(), key=lambda item: (not item.is_dir(), item.name.lower())):
            ...
            if is_hidden(rel) or rel.parts[0] == "_dashboard":
                continue
            if child.is_symlink():
                # 符号链接均为兼容构造（如 _staging、跨分类封面链接），
                # 跟随会造成内容重复甚至目录环，扫描时统一跳过。
                continue
            try:
                stat = child.stat()
            except OSError:
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def serve_app(self, request_path: str) -> None:
        ...
        if not relative:
            ...
            path = self.safe_child(ASSETS_DIR, relative)
            if path is None:
...
def main() -> int:
    global OUTPUTS_ROOT, CACHE_PATH
    parser = argparse.ArgumentParser(description="Offline dashboard for a content folder")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8765)
    parser.add_argument("--root", default=str(DEFAULT_ROOT),
                        help="Directory to browse (default: repo outputs/)")
    ...
    OUTPUTS_ROOT = Path(args.root).expanduser().resolve()
    if not OUTPUTS_ROOT.is_dir():
        raise SystemExit(f"Not a directory: {OUTPUTS_ROOT}")
    CACHE_PATH = OUTPUTS_ROOT / ".dashboard-cache.json"
    load_cache()
