(() => {
  ...
  function indexTree(root) {
    ...
    projects.sort((a, b) => b.quality - a.quality || b.mtime - a.mtime);
  ...
  function renderChrome() {
    ...
    const tags = [
      { key: "all", label: "全部作品", icon: "✦", desc: "浏览所有项目" },
      ...TAGS,
      TAG_OTHER,
    ];
    const current = route();
    const activeTag = current.type === "tag" ? current.key : current.type === "home" && !state.query ? "all" : "";
    els.tagNav.innerHTML = tags.map((tag) => `
      <a class="tag-link ${activeTag === tag.key ? "active" : ""}" href="#/tag/${encodeURIComponent(tag.key)}">
        <span class="tag-link-icon">${tag.icon}</span>
        <span class="tag-link-label">${tag.label}</span>
        <em>${tag.key === "all" ? state.projects.length : (tagCounts.get(tag.key) || 0)}</em>
      </a>
    `).join("");
  ...
  function ownerPath(path) {
    return path === "__root__" || !path.includes("/") ? (state.projectByPath.has(path) ? path : "__root__") : path.split("/")[0];
  }
  ...
  function renderHome() {
    ...
    els.content.innerHTML = `
      <section class="hero">
        ...
        <h1>按标签与项目整理的离线作品库</h1>
        <p>自动识别视频、图片、绘本、报告、网页、音频、代码与实验数据。摘要在本地预生成并缓存，新增内容刷新即可出现，无需重新生成页面。</p>
      ...
      <section class="section">
        <div class="section-head"><div><h2>标签分类</h2><p>点击标签后按目录缩略浏览同类项目</p></div></div>
        <div class="tag-grid">
          ${[...TAGS, TAG_OTHER].map((tag) => {
  ...
  function renderProject(project) {
    els.breadcrumbs.innerHTML = `<a href="#/">Outputs</a><span>›</span><a href="#/tag/${encodeURIComponent(project.tag)}">${tagMeta(project.tag).label}</a><span>›</span><span>${escapeHtml(project.name)}</span>`;
    els.content.innerHTML = `
